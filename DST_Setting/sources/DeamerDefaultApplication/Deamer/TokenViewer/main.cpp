...
#include "DST_Setting/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DST_Setting::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DST_Setting::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DST_Setting::ast::relation::ConvertEnumToText(static_cast<DST_Setting::ast::Type>(token->Type)) << "\n";
