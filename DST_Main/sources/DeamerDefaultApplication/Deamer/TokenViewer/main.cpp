...
#include "DST_Main/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DST_Main::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DST_Main::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DST_Main::ast::relation::ConvertEnumToText(static_cast<DST_Main::ast::Type>(token->Type)) << "\n";
