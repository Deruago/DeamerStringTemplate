...
#include "DST/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DST::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DST::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DST::ast::relation::ConvertEnumToText(static_cast<DST::ast::Type>(token->Type)) << "\n";
